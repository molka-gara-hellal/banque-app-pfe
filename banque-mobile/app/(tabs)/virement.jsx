    ...
    return (
      <View style={s.successContainer}>
        ...
        <View style={s.successCard}>
          ...
          <Text style={s.successAmt}>{parseFloat(montant).toFixed(2)} TND</Text>
      ...
      <View style={s.confirmContainer}>
        <View style={s.confirmCard}>
          ...
          <View style={s.confirmAmtRow}>
            ...
            <Text style={s.confirmAmt}>{montantNum.toFixed(2)} TND</Text>
          </View>
        </View>
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView style={s.container} contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.card}>
          ...
          <View style={s.quickRow}>
            {["50", "100", "200", "500"].map(v => (
              <TouchableOpacity
                key={v}
                style={[s.quickBtn, montant === v && s.quickBtnActive]}
        ...
        <View style={s.card}>
          ...
          <TextInput
            style={s.motifInput}
            placeholder="Ex: Loyer, Prêt, Remboursement..."
            value={motif}
        ...
        <TouchableOpacity
          style={[s.sendBtn, !isFormValid && s.btnDisabled]}
          onPress={handlePreview}
          disabled={!isFormValid}
        >
          <Text style={s.sendBtnIcon}>↗</Text>
          <Text style={s.sendBtnText}>
            Vérifier le virement{montant ? ` (${parseFloat(montant || 0).toFixed(2)} TND)` : ""}
          </Text>
        </TouchableOpacity>
      </ScrollView>
    </KeyboardAvoidingView>
